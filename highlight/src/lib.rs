...
impl HighlightConfiguration {
    ...
    pub fn new(
        ...
        let mut query = Query::new(language.clone(), &query_source)?;
        ...
        let mut combined_injections_query = Query::new(language.clone(), injection_query)?;
    ...
    fn new<F: FnMut(&str) -> Option<&'a HighlightConfiguration> + 'a>(
        ...
        loop {
            if highlighter.parser.set_included_ranges(&ranges).is_ok() {
                highlighter
                    .parser
                    .set_language(config.language.clone())
